...
def get_id_path_(tax_id, ncbi_info):
    path = get_id_path(tax_id, ncbi_info.tax_id_to_parent, ncbi_info.tax_id_to_rank, ncbi_info.tax_id_to_tax_id)
    path[-1] = tax_id
    return path

class TaxonomicQuery():
    def __init__(self):
        self.tax_id_to_parent = None
        self.tax_id_to_rank = None
        self.tax_id_to_name = None
        self.tax_id_to_tax_id = None

def load_ncbi_info(ncbi_nodes_file, ncbi_names_file, ncbi_merged_file):
    if ncbi_nodes_file:
        binning_classes = TaxonomicQuery()
        binning_classes.tax_id_to_parent, binning_classes.tax_id_to_rank = load_tax_info(ncbi_nodes_file)
        if ncbi_names_file:
            binning_classes.tax_id_to_name = load_names(binning_classes.tax_id_to_rank, ncbi_names_file)
        if ncbi_merged_file:
            binning_classes.tax_id_to_tax_id = load_merged(ncbi_merged_file)
        return binning_classes