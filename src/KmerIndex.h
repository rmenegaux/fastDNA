...
//  bool matchEnd(const char *s, int l, std::vector<std::pair<int, int>>& v, int p) const;
  ...
  // load methods
  // CHANGE
  void load(const std::string& index_in, bool loadKmerTable = true);
  ...
  const size_t INDEX_VERSION = 11; // increase this every time you change the fileformat
